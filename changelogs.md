...
### Title
- Uncategorized filter and Episodes admin UX improvements

### Added
- Episodes admin “Category” filter now includes an **Uncategorized** option to list only episodes without any internal Podify category assignment.

### Changed
- Episodes admin filter bar now passes an `uncategorized` flag through the REST API and database layer so uncategorized filtering works consistently across pagination and search.
- Episode queries now support explicit filtering for “no category” using a LEFT JOIN on the episode–category mapping table.
- Admin checkbox and radio inputs use unified sizing and accent color for a consistent look across filters and bulk selectors.
