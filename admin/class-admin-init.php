...
class AdminInit {
    ...
    public static function page() {
        ...
        if ($tab === 'import') {
            ...
            $limit_ep = $feed_filter ? (isset($_GET['limit']) ? max(25, min(200, intval($_GET['limit']))) : 50) : 10;
            ...
            if ($feed_filter) {
                echo '<div class="podify-field" style="margin-top:0"><strong>Feed '.$feed_filter.':</strong> showing episodes for this feed. <a href="'.$base.'&tab=episodes&feed_id='.$feed_filter.'&limit=200">Show all for this feed</a></div>';
                echo '<div class="podify-filter-bar">';
                echo '<div class="podify-field"><label>Items per page</label><select id="podify-ep-limit"><option value="25"'.($limit_ep===25?' selected':'').'>25</option><option value="50"'.($limit_ep===50?' selected':'').'>50</option><option value="100"'.($limit_ep===100?' selected':'').'>100</option><option value="200"'.($limit_ep===200?' selected':'').'>200</option></select></div>';
            ...
            echo '<table id="podify-admin-episodes" class="widefat" data-feed="'.$feed_filter.'" data-offset="'.$offset_cur.'" data-limit="'.$limit_ep.'" data-page="1"><thead><tr><th>Title</th><th>Feed</th><th>Published</th><th>Audio URL</th><th>Image URL</th><th>Category</th></tr></thead><tbody>';
            if ($episodes) {
                foreach ($episodes as $e) {
                    ...
                    $select = '<select class="podify-assign-cat" data-episode="'.intval($e['id']).'"><option value="">— Select —</option>';
                    if ($cats) {
                        foreach ($cats as $c) {
            ...
            echo '</div>';
            if ($feed_filter) {
                echo '<div class="podify-actions"><button class="button" id="podify-admin-prev" disabled>Prev</button> <span id="podify-admin-page">Page 1</span> <button class="button" id="podify-admin-next"'.(($offset_cur<$limit_ep)?' disabled':'').'>Next</button></div>';
            }
            ...
            if ($feed_filter) {
                ...
                echo 'function makeCatSelect(epId){ var html = \'<select class="podify-assign-cat" data-episode="\'+epId+\'"><option value="">— Select —</option>\'; PODIFY_CATS.forEach(function(c){ html += \'<option value="\'+(c.id||0)+\'">\'+(c.name||"")+\'</option>\'; }); html += \'</select><div class="podify-assigned">Assigned: —</div>\'; return html; }';
                echo 'function setPageLabel(p){ var el=document.getElementById("podify-admin-page"); if(el){ el.textContent="Page "+String(p); } }';
